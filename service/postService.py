...
def createPost(image_bytes: Optional[bytes], image_mime: Optional[str], text: str, user_id: int) -> PostResponseDTO:
    post_id = save_post(
...
def searchPostByText(word: str) -> list[PostResponseDTO]:
    ...
    return [_post_to_dto(*post) for post in posts]
